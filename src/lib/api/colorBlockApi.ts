...
import {
  ...
  type ColorBlockPresetCacheKey
} from '../../constants/colorBlocks';
...
export function getColorBlockApiBase(): string {
  ...
  return (configured?.trim() || COLOR_BLOCK_API_BASE).replace(/\/+$/, '');
}
...
export function getColorBlockPresetCacheKey(camp: string): ColorBlockPresetCacheKey | null {
  ...
  if (COLOR_BLOCK_SPECIFIC_CAMPS.has(normalizedCamp)) return normalizedCamp as ColorBlockPresetCacheKey;
  if (!normalizedCamp || COLOR_BLOCK_ALL_CAMPS.has(normalizedCamp)) return 'all';
  return null;
}
