...
import { decorationAtlasFrames, gafSources } from '../../mock/gafManifest';
...
function makeOptionSprite(option: PartOption | undefined, failedTextures: Set<string>): Sprite | null {
  ...
  if (failedTextures.has(option.atlas.texture)) return null;
...
/**
 * Canonical decoration visual used by both the interactive workspace and
 * off-screen exports. Keeping asset selection, GAF fallback, registration and
 * display transforms here prevents preview/export drift.
 */
export function applyDecorationVisualTransform(wrapper: Container, deco: DecorationLayer): void {
...
export function createDecorationVisual(deco: DecorationLayer, failedTextures: Set<string>): Container | null {
  ...
  applyDecorationVisualTransform(wrapper, deco);
  ...
  if (missing) {
    ...
    const fallback = option.atlas
      ? [option.atlas]
      : decorationAtlasFrames[linkage]
        ? [decorationAtlasFrames[linkage]]
        : [];
