...
export const zhTW: Record<string, string> = {
  ...
  'autoCreate.section.learning': '候選排序學習',
  'autoCreate.learning.enabled': '啟用 MLP 學習與已核准的候選排序（未通過 Full gate 時維持原版搜尋）',
  'autoCreate.learning.rankerLab': '本機試用已訓練排序器',
  'autoCreate.learning.rankerLabWarning': '本機實驗已繞過 rollout gate；模型：{revision}；模式：{modes}。此結果不代表正式核准。',
  'autoCreate.learning.exportPortable': '匯出本機訓練資料',
  'autoCreate.learning.importPortable': '匯入本機模型',
  'autoCreate.learning.cancelExport': '取消匯出',
  'autoCreate.learning.portableUnsupported': '此瀏覽器不支援直接選擇資料夾，請使用本機Chrome／Chromium。',
  'autoCreate.learning.exportProgress': '已掃描 {scanned} 筆；已匯出 {exported} 筆；{shards} 個分片。',
  'autoCreate.learning.exportDone': '訓練資料匯出完成：{count} 筆，{shards} 個分片。',
  'autoCreate.learning.importDone': '本機模型匯入完成：{revision}',
  'autoCreate.learning.summary': '狀態：{phase} · Add：{add} · Replace：{replace} · 模型：{revision}',
  'autoCreate.learning.fallback': '排序器已回退原版搜尋：{reason}',
  'autoCreate.learning.clear': '清除此陣營的學習資料',
  'autoCreate.learning.working': '處理中…',
  'autoCreate.learning.clearWarning': '學習紀錄已清除，但部分 TensorFlow.js 模型檔案未能移除：{message}',
